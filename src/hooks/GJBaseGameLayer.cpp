...
#if __has_include(<chizz.subtick-inputs-api/include/SubtickInputs.hpp>)
#include <chizz.subtick-inputs-api/include/SubtickInputs.hpp>
#define SILICATE_HAS_SUBTICK 1
#elif __has_include(<SubtickInputs.hpp>)
...
struct SLGJBaseGameLayer : Modify<SLGJBaseGameLayer, GJBaseGameLayer> {
    ...
    void processQueuedButtons(float dt, bool clearInputQueue) {
        auto bot = Bot::get();
        if (!bot->isEnabled()) {
#ifdef SILICATE_HAS_SUBTICK
            if (geode::Loader::get()->isModLoaded("chizz.subtick-inputs-api")) {
                if (!subtickinputs::prelude::useVanilla()) {
                    subtickinputs::prelude::processInputs(dt);
                }
            }
#endif
